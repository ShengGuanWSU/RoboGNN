...
def new_get_train_val_test(nnodes, G, graph_name):
    """This setting follows nettack/mettack, where we split the nodes
    into 10% training, 10% validation and 80% testing data
    ...
    if graph_name.find("cora")!=-1:
        print("The input dataset is cora ........")
        idx_train = np.arange(140)
        idx_val = np.empty(1,dtype=int)
        idx_test = np.empty(1,dtype=int)
        for node in G.nodes():
            if node in idx_train:
                print("Train,currrent n is %d" % node)
            elif G.nodes[node]['test'] == False and G.nodes[node]['val'] == False:
                print("no label id,currrent n is %d" % node)
            elif G.nodes[node]['test'] == False and G.nodes[node]['val'] == True:
                print("Validation, current n is %d" % node)
                idx_val= np.concatenate((idx_val,[node]),axis=0)
            elif G.nodes[node]['test'] == True and G.nodes[node]['val'] == False:
                print("Test, current n is %d" % node)
                idx_test= np.concatenate((idx_test,[node]),axis=0)
        idx_val =  np.delete(idx_val, 0, axis=0)
        idx_test = np.delete(idx_test, 0, axis=0)
        print(idx_val.shape)
        print(idx_test.shape)

        return idx_train, idx_val, idx_test
    if graph_name.find("citeseer")!=-1:
        print("The input dataset is citeseer ........")
    ...
    if graph_name.find("dbpedia") != -1:
        print("The input dataset is dbpedia ........")
        nodes_num = G.number_of_nodes()
        semi_threshold = int(round(nodes_num * 0.6 * 0.1))
        print("The graph |V_T| is %d" % semi_threshold)
        idx_train = np.arange(semi_threshold)
        idx_val = np.empty(1, dtype=int)
        idx_test = np.empty(1, dtype=int)
        for node in G.nodes():
            if node in idx_train:
    ...
    if graph_name.find("yelp") != -1:
        print("The input dataset is yelp ........")
        nodes_num = G.number_of_nodes()
        semi_threshold = int(round(nodes_num * 0.6 * 0.1))
        print("The graph |V_T| is %d" % semi_threshold)
        idx_train = np.arange(semi_threshold)
    ...
    if graph_name.find("oag") != -1:
        print("The input dataset is oag ........")
        nodes_num = G.number_of_nodes()
        semi_threshold = int(round(nodes_num * 0.6 * 0.1))
        print("The graph |V_T| is %d" % semi_threshold)
        idx_train = np.arange(semi_threshold)
...
def cal_scores(output, labels):
    """Return precision, recall and fscore of output compared to labels.
    ...
    #print(labels.device)
    #print(preds.device)
    precision, recall, fscore, support = metrics.precision_recall_fscore_support(labels.cpu(), preds.cpu(), average='macro')
    balanced_accuracy_score = metrics.accuracy_score(labels.cpu(), preds.cpu(), normalize=True)

    return precision, recall, fscore, support, balanced_accuracy_score